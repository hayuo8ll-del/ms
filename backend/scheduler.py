...
# The web page is published from the repo-root ``mlb/`` folder so GitHub Pages
# (which serves the ``main`` branch) exposes it at ``/ms/mlb/`` alongside the
# existing site at ``/ms/``. It is committed (not gitignored) for branch deploy.
DEFAULT_SITE_DIR = Path(__file__).resolve().parent.parent / "mlb"
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        "GitHub Pages at /ms/mlb/ (default: repo-root mlb/)",
    )
    return parser
